...
%     log.n_subjs=length(log.subjects);
    ...
    [w.a instruc.cells w.b]=xlsread([where.analysisscript filesep 'I_memanalysis_instruc.xlsx'], log.taskversion);
...
ws.old(:,22:29)=nan;
disp('turned off mark parameeters')
    %
...
for s=1:log.n_subjs
    ...
    for t=1:size(ws.d,1)
        ...
        for o6=1:1 % EVExplore
            ...
            % EVExplore Third
             if ws.d(t,col.m.EVExplore)<=design.var.EVExplore(5)
                ws.d(t,col.m.EVExplore_Third)=1;
            elseif ws.d(t,col.m.EVExplore)<=design.var.EVExplore(10) && ws.d(t,col.m.EVExplore)>design.var.EVExplore(5)
                ws.d(t,col.m.EVExplore_Third)=2;
            elseif ws.d(t,col.m.EVExplore)>design.var.EVExplore(10)
                ws.d(t,col.m.EVExplore_Third)=3;
            else
                error('Unidentified EVExplore value!')
            end
        end
        ...
        for o7=1:1 % EV
            % EV Median
             if ws.d(t,col.m.EV)<=design.var.EV(7)
                ws.d(t,col.m.EV_Med)=1;
            elseif ws.d(t,col.m.EV)>design.var.EV(7)
                ws.d(t,col.m.EV_Med)=2;
            else
                error('Unidentified EV value!')
             end
            
             % EV Third
                if ws.d(t,col.m.EV)<=design.var.EV(5)
                ws.d(t,col.m.EV_Third)=1;
            elseif ws.d(t,col.m.EV)<=design.var.EV(9) && ws.d(t,col.m.EV)>design.var.EV(5)
                ws.d(t,col.m.EV_Third)=2;
            elseif ws.d(t,col.m.EV)>design.var.EV(9)
                ws.d(t,col.m.EV_Third)=3;
            else
                error('Unidentified EV value!')
            end
        end
        ...
        % In Cluster
        if  and(ws.d(t,col.m.EnvThreat_Level)==2 || ws.d(t,col.m.EnvThreat_Level)==3  , ws.d(t,col.m.NTokens_Level)==3 || ws.d(t,col.m.NTokens_Level)==4)
            ws.d(t,col.m.InCluster)=1;
        end
         
        % Explore
        if ws.d(t,col.m.Choice)==1 || ws.d(t,col.m.Choice)==2
            ws.d(t,col.m.Choice_Explore)=0;
        elseif ws.d(t,col.m.Choice)==3
            ws.d(t,col.m.Choice_Explore)=1;
        end
       
        
    end 
    
    subjdata{s,2}.olds=ws.d;
    ws=[];
...
% FA rate
for s=1:log.n_subjs
    subjdata{s,3}.FA.hit=sum(subjdata{s,2}.foils(:,col.m.OldNew)==1)/size(subjdata{s,2}.foils,1);
    subjdata{s,3}.FA.surehit=sum(subjdata{s,2}.foils(:,col.m.OldNew)==1 & subjdata{s,2}.foils(:,col.m.SureGuess)==1)/size(subjdata{s,2}.foils,1);
    subjdata{s,3}.FA.rem=sum(subjdata{s,2}.foils(:,col.m.RemKnow)==1)/size(subjdata{s,2}.foils,1);
    subjdata{s,3}.FA.know=sum(subjdata{s,2}.foils(:,col.m.RemKnow)==2)/size(subjdata{s,2}.foils,1);
    subjdata{s,3}.FA.surerem=sum(subjdata{s,2}.foils(:,col.m.RemKnow)==1 & subjdata{s,2}.foils(:,col.m.SureGuess)==1)/size(subjdata{s,2}.foils,1);
    subjdata{s,3}.FA.sureknow=sum(subjdata{s,2}.foils(:,col.m.RemKnow)==2 & subjdata{s,2}.foils(:,col.m.SureGuess)==1)/size(subjdata{s,2}.foils,1);
    subjdata{s,3}.FA.Pos1=sum(subjdata{s,2}.foils(:,col.m.Pos)==1)/size(subjdata{s,2}.foils,1);
    subjdata{s,3}.FA.Pos2=sum(subjdata{s,2}.foils(:,col.m.Pos)==2)/size(subjdata{s,2}.foils,1);
    subjdata{s,3}.FA.Pos3=sum(subjdata{s,2}.foils(:,col.m.Pos)==3)/size(subjdata{s,2}.foils,1);
    subjdata{s,3}.FA.Pos4=sum(subjdata{s,2}.foils(:,col.m.Pos)==4)/size(subjdata{s,2}.foils,1);
end

% Set up results tables
% w.res=cell(log.n_subjs+1, size(instruc.cells,1)+1); % Tables
% w.res(:,1)=[{'Subject'}; log.subjects]; w.res(1,2:end)=instruc.cells(:,1)';
% t_dprime=w.res; t_hit=w.res; t_surehit=w.res; t_assoc=w.res;
% t_rem=w.res; t_know=w.res; t_surerem=w.res; t_sureknow=w.res;
% t_phitsure=w.res; t_phitrem=w.res; t_phitsurerem=w.res; t_premsure=w.res; t_pknowsure=w.res;
% w.grid=repmat({nan*zeros(log.n_subjs,1)},design.EnvThreat_Levels, design.NTokenPairs_Levels);  % Data grids
% d_dprime=w.grid; d_hit=w.grid; d_surehit=w.grid; d_assoc=w.grid;
% d_rem=w.grid; d_surerem=w.grid; d_know=w.grid; d_sureknow=w.grid;
% d_phitsure=w.grid; d_phitrem=w.grid; d_phitsurerem=w.grid; d_premsure=w.grid; d_pknowsure=w.grid;
% d_a_dprime=w.grid; d_a_hit=w.grid; d_a_surehit=w.grid; d_a_assoc=w.grid;
% d_a_rem=w.grid; d_a_surerem=w.grid; d_a_know=w.grid; d_a_sureknow=w.grid;
...
t_phitsure=w.res; t_phitrem=w.res; t_phitsurerem=w.res; t_premsure=w.res; t_pknowsure=w.res;
% w.grid=repmat({nan*zeros(log.n_subjs,1)},design.EnvThreat_Levels, design.NTokenPairs_Levels);  % Data grids
w.grid=repmat({cell(log.n_subjs,1)},design.EnvThreat_Levels, design.NTokenPairs_Levels);  % Data grids
d_dprime=w.grid; d_hit=w.grid; d_surehit=w.grid; d_assoc=w.grid;
d_rem=w.grid; d_surerem=w.grid; d_know=w.grid; d_sureknow=w.grid;
d_phitsure=w.grid; d_phitrem=w.grid; d_phitsurerem=w.grid; d_premsure=w.grid; d_pknowsure=w.grid;
d_a_dprime=w.grid; d_a_hit=w.grid; d_a_surehit=w.grid; d_a_assoc=w.grid;
...
for s=1:log.n_subjs
    ...
    subjdata{s,3}.celldata=cell(size(instruc.cells,1),2); % data sorted into cells (col 1=name, col 2= trialstats)
        ...
        disp('fake inputs')
        ws.d(:,col.m.EnvThreat_Level)=randi(6,size(ws.d,1),1);
        ws.d(:,col.m.NTokens_Level)=randi(6,size(ws.d,1),1);  % Fake inputs
        
        
    % ##########################################
    % Automated sort: simpler version
    for i=1:size(instruc.cells,1)
        disp(['Cell: ' instruc.cells{i,1}])
        
        % Compile sample
        wi.sample=[];
        for crit=1:size(instruc.cells{i,2},1)
            wi.newd=['ws.d(ws.d(:, col.m.' instruc.cells{i,2}{crit,1} ')==' num2str(instruc.cells{i,2}{crit,2})     ',:)'];
            wi.sample=[wi.sample; wi.newd];
        end
    end
     % ##########################################
...
design.EnvThreat_labels={'4/4' '3/4' '2/4' '1/4'};
for i=1:length(design.EnvThreat_pBomb)
%     [n d]=rat(design.EnvThreat_pBomb(i));
%     design.EnvThreat_labels{i}=[ num2str(n) '/' num2str(d)];
end
