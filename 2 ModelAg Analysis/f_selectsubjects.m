...
% Execute following to debug with script:   olddatalog=log.datalog; specificsubjects=log.specificsubjects; koshertable=log.koshertable; 
% modelname=[request.onsetsmodel '_' request.memtype];
...
olddatalog(:,1)=strtrim(olddatalog(:,1));
...
for i=excel.firstsubjectname_row:size(koshertable,1); 
    if koshertable{i, excel.firstmodel_col-1+find(strcmp(modelname,allmodels))}==1
        kosherlist{j,1}=strtrim(koshertable{i,1});
        j=j+1;
    end
end
        ...
        for j=2:size(olddatalog,1)
            if strcmp(olddatalog{j,1}, subjectlist{i})==1
                for k=1:size(olddatalog,2)
                    newdatalog{i+1,k}=olddatalog{j,k};
                end
                w.sok=1;
            end
