% Execute conflict-encoding task (trial parameters set up previous script)
...
for trialnum=1: p.nTrials
    ...
    disp(['Item stim: ' num2str(rdata( trialnum, col.ItemStim))  '   -  ' itemlist{rdata(trialnum, col.ItemStim),1} '  (type ' num2str(rdata(trialnum, col.ItemType)) ')']);
    disp(['Item pos: '  num2str(rdata(trialnum, col.ItemPos))]) 
    ...
    for o1=1:1 % Set up displays & details for this trial
        ...
        % Image display sprite (sprite #2)
        cgloadbmp(2,itemlist{rdata(trialnum, col.ItemStim),1},p.disp.ItemSize,p.disp.ItemSize)
        eval(['wt.item_x=p.disp.ItemPos' num2str(rdata(trialnum,col.ItemPos)) '_x;'])
        eval(['wt.item_y=p.disp.ItemPos' num2str(rdata(trialnum,col.ItemPos)) '_y;'])
        %
        
        % Which hand?
        if rdata(trialnum, col.ItemType)==1 % Natural = Right hand
            wt.key1=p.keys.right1;
            wt.key2=p.keys.right2;
            wt.key3=p.keys.right3;
        elseif rdata(trialnum, col.ItemType)==2 % Manmade = Left hand
            wt.key1=p.keys.left1;
            wt.key2=p.keys.left2;
            wt.key3=p.keys.left3;
        end
    ...
    cgdrawsprite(sprite.ResponseOptions,0,-250)
    cgdrawsprite(2,wt.item_x,wt.item_y);
    %
    waituntil(wt.offer1only*1000+p.disp.time_firstofferonly)
    clearkeys
    wt.offer1item=cgflip(w.back);
    [wk.key1press wk.key1time wk.key1n]=waitkeydown(p.disp.time_firstofferonly,[p.keys.left1 p.keys.left2 p.keys.left3 p.keys.right1 p.keys.right2 p.keys.right3]);
    %
    if wk.key1n==0 % No response
        cgdrawsprite(sprite.EnvThreat_Offers(rdata(trialnum,col.EnvThreat)),0,0)
        for j=1:rdata(trialnum,col.NTokenPairs)
            cgdrawsprite(sprite.TokenActive,wt.posX(j),p.disp.tokenY_row1);
            cgdrawsprite(sprite.TokenActive,wt.posX(j),p.disp.tokenY_row2);
        ...
        waituntil(wt.offer1item*1000+p.disp.time_firstofferitem)
        wt.outcome=cgflip(w.back);
        rdata(trialnum,col.Trialvalid)=0;
    elseif  wk.key1press(1)==wt.key1 || wk.key1press(1)==wt.key2 || wk.key1press(1)==wt.key3
        rdata(trialnum,col.Trialvalid)=1; % Log responses (with correct hand)
        switch wk.key1press(1)
            case wt.key1
                rdata(trialnum,col.Resp1)=1;
            case wt.key2
                rdata(trialnum,col.Resp1)=2;
            case wt.key3
                rdata(trialnum,col.Resp1)=3;
        end
        rdata(trialnum,col.RT1)= wk.key1time(1)-wt.offer1item*1000;
    else
        rdata(trialnum,col.Resp1)=-1;
    ...
    if rdata(trialnum,col.Resp1)==3
        ...
        [wk.key2press wk.key2time wk.key2n]=waitkeydown(p.disp.time_secondoffer, [wt.key1 wt.key2]);
        if wk.key2n==0 % No response
            ...
            %
            cgtext('NO RESPONSE',0,160)
            waituntil(wt.offer2*1000+p.disp.time_secondoffer)
            wt.outcome=cgflip(w.back);
            rdata(trialnum,col.Trialvalid)=0;
        else
    ...
    cgtext('+',0,0)
    waituntil(wt.outcome*1000+p.disp.time_outcome)
...
outcomes.itemerrors=itemerrors;

% Save
p.log.date=date;
cF.rdata=rdata;
cF.data=rdata(rdata(:,col.Trialvalid)==1,:);
cF.par=p;
cF.col=col;
...
save(['Data' filesep p.subject '_file_3ExploreEnc.mat'], 'cF');
try % Transfer file to DeletedDaily 
    ...
    copyfile(['Data' filesep p.subject '_file_3ExploreEnc.mat'],  [movetowhere filesep p.subject '_file_3ExploreEnc.mat']);
    w.warning=' ';
catch
...
disp(['# Omissions: ' num2str(outcomes.n_omissions) ' (of ' num2str(size(rdata,1)) ')'])
disp(['# Item errors: ' num2str(itemerrors) ' (of ' num2str(size(rdata,1)) ')'])
