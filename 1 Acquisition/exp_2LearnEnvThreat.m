...
% error
% rdata(1,col.BombPresent)
% rdata(1,col.BombActivated)
...
for trialnum=1: p.nTrials
    ...
    disp(['Present Bomb=' num2str(rdata(trialnum,col.BombPresent))])
    ...
    for o1=1:1 % Set up displays & details for this trial
        ...
        % Shuffle positions of Activated tokens (display)
        ...
        wt.posX(1:rdata(trialnum,col.NTokenPairs),1)=wt.pos((wt.pos(:,2)==1),1); % Positions in which to mark activated tokens
         ...
         % Shuffle position of Inactivated Bombs (if any, constrained by activated tokens) 
        if rdata(trialnum, col.BombPresent)==1 && rdata(trialnum, col.BombActivated)==0 
            wt.inactivepos=sortrows([wt.pos(wt.pos(:,2)==0,1) rand(sum(wt.pos(:,2)==0),1)],2);
            eval(['wt.inactivebombpos=[wt.inactivepos(1) p.disp.tokenY_row' num2str(randi(2)) '];'])
        end
    end
...
%     wt.offer1item=cgflip(w.back);
    ...
    if rdata(trialnum,col.Trialvalid)==1
        ...
        % Display inactivated bombs (if applicable)
        if rdata(trialnum, col.BombPresent)==1 && rdata(trialnum, col.BombActivated)==0 
            cgdrawsprite(sprite.EnvThreat_Offers(rdata(trialnum,col.EnvThreat)),0,0)
            for j=1:rdata(trialnum,col.NTokenPairs)
                cgdrawsprite(sprite.TokenActive,wt.posX(j),p.disp.tokenY_row1);
            ...
            cgtext(wt.infocost,0,-130)
            cgtext('+',0,0)
            cgdrawsprite(sprite.Explored_Bomb,wt.inactivebombpos(1),wt.inactivebombpos(2)); % Inactivated bomb
            waituntil(wt.outcome*1000+p.disp.time_firstofferonly/2)
            wt.outcome=cgflip(w.back);
        end
    end
    ...
    if rdata(trialnum,col.MonitorTask)==1
        ...
        cgfont('Helvetica',40)
        cgtext(wt.infocost,0,-130)
        if rdata(trialnum, col.BombPresent)==1 && rdata(trialnum, col.BombActivated)==0  % Inactivated bomb if shown
            cgdrawsprite(sprite.Explored_Bomb,wt.inactivebombpos(1),wt.inactivebombpos(2)); 
        end
        %        
        cgpencol(1,1,1);
        cgtext('+',0,0)
